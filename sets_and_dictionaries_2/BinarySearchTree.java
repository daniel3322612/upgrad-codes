...
public class BinarySearchTree {
    ...
    //O(n) O(w)  Breadth First Traversal (Or Level Order Traversal)
    private void bsf(Node node) {
        if (node == null) {
            return;
        }

        Queue<Node> queue = new Queue<>(Node.class, 10);
        queue.enqueue(node);
        while (!queue.isEmpty()) {
    ...
    public void printKDistantNodes(Node node, int k) {
        //Base case
        if (node == null || k < 0)
            return;
        if (k == 0) {
